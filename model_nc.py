...
class BaseConvModel(nn.Module):
    def __init__(self, exp, CNN_only=False):
        ...
        if self.CNN_only == True:
            ...
            self.conv1 = nn.Conv2d(exp_var_dict['img_input_channels'], exp_var_dict['proto_input_channels'], kernel_size=(3,3), stride=1, padding='same')
...
#script modified from Barnes EA et. al (2022) doi: 10.1175/AIES-D-22-0001.1 and Chen C et al (2019) doi: 10.48550/arXiv.1806.10574 
class ProtoModel(nn.Module):
    ...
    def forward(self, x):
        ...
        min_distances = torch.amin(distances / (location_scale_factor + self.epsilon), dim=(2,3)) #maxpool2d in protop
...
class WholeModel(nn.Module):
    def __init__(self, exp, num_channels):
        super().__init__()
    ...
    def forward(self, x, enable_dropout=True):
        ...
        for chan in range(self.num_channels):
            output_chan = self.base_list[chan](x[:,:,:,:], enable_dropout)
